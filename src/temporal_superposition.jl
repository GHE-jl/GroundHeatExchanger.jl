...
# Output
    - Convolved signal. Typically temperature response (nₜ x 1) [°C].
"""
function convolution_time(q::AbstractVector{T}, g::AbstractVector{T}) where {T<:AbstractFloat}
    n = length(q)           # Number of time steps
    f = impulse_func(q)     # Impulse function of the thermal loads
    y = zeros(T, n)         # Preallocation
    
    # Loop over time steps
    for i in 1:n
        # Calculate the sum for each point i
        for j in 1:i
            y[i] += f[i - j + 1] * g[j]
        end
    end
...
"""
    convolution_matrix(t, q, g)

Function that performs temporal superposition through a convolution product solved in the time
domain using matrix multiplication. This function is not optimized for large vectors and is only
provided for small vector with even or uneven time steps. O(n²) complexity.
# Arguments
    - `q`: Load functions (nₜ x 1) [W/m, W, °C]
    - `g`: Transfer function (or g-function) (nₜ x 1) [°Cm/W, °C/W, -]
# Output
...
"""
function convolution_matrix(q::AbstractVector{T}, g::AbstractVector{T}
    ) where {T<:AbstractFloat}
    n = length(q)           # Number of time steps
    f = impulse_func(q)     # Impulse function of the thermal loads
    A = zeros(T, n, n)      # Preallocate dense matrix
    ...
    # Use LowerTriangular for optimized multiplication
    return LowerTriangular(A) * g
end
...
# Arguments
    - `y`: Output vector for in-place convolution (nₜ x 1) [°C].
    - `f`: Incremental thermal load function (nₜ x 1) [W, W/m, °C] (incremental_func())
    - `g`: Transfer function (or g-function) (nₜ x 1) [°C/W, °Cm/W, -]
    - `plan`: Optional precomputed FFT plan for the convolution.
# Output
    - `y`: Convolved signal. Typically temperature response (nₜ x 1) [°C].
# Reference
    - Marcotte, D., & Pasquier, P. (2008). Fast fluid and ground temperature computation for 
        geothermal ground-loop heat exchanger systems. Geothermics, 37(6), 651–665.
        https://doi.org/10.1016/j.geothermics.2008.08.003
    - Pasquier, P., & Marcotte, D. (2013). Efficient computation of heat flux signals to ensure the
        ...
        Engineering, 59(1–2), 515–526. https://doi.org/10.1016/j.applthermaleng.2013.06.018
# Example
    plan = plan_rfft(zeros(length(Q), nextprod([2, 3, 5], 2 * length(Q) - 1))) # Precompute FFT plan
...
function convolution_FFTW!(y::AbstractVector{T}, f::AbstractVector{T}, g::AbstractVector{T};
    ...
    n = length(f)
    n == length(g) || throw(ArgumentError("Vectors f and g must have the same length"))
    n == length(y) || throw(ArgumentError("Output vector y must have the same length as f and g"))

    # Optimal padding length for convolution (next power of 2 or product of small primes)
    pad = nextprod([2, 3, 5], 2 * n - 1)

    # Pad f and g with zeros to avoid circular convolution
    f_pad = zeros(T, pad)
    g_pad = zeros(T, pad)
    f_pad[1:n] .= f
    g_pad[1:n] .= g

    # Check if a precomputed FFT plan is provided and compatible
    if isnothing(plan)
        plan = plan_rfft(zeros(T, pad))
    else
        size(plan) == (pad,) || throw(ArgumentError(
            " Provided FFT plan not compatible with the required padding length"))
    end

    # Compute FFT of f and g with padding
    F_f = plan * f_pad
    F_g = plan * g_pad

    # Frequency domain multiplication (In-place)
    F_f .*= F_g
