...
temperature, and can be computed with functions `water_ρ` and `water_μ` respectively.
# Arguments
...
equation. The function uses an iterative method to solve the implicit equation.
# Arguments
    - `Re`: Reynolds number [-]
    - `r`: Pipe inside or annulus (r = rb - ro) radius [m]
    - `ϵ`: Pipe roughness [m]
...
"""
    Nusselt(Re, Pr, r, ϵ=0.0)
    Nusselt(V̇, r, kf, cf, ρf, μf, ϵ-0.0)
...
# Output
    - `Nu`: Nusselt number [-]
"""
function Nusselt(Re::Real, Pr::Real, r::Real, ϵ::Real=0.0)
...
function Nusselt(V̇::Real, r::Real, kf::Real, cf::Real, ρf::Real, μf::Real, ϵ::Real=0.0)
...
"""
    ...
    resistance_fluid(V̇, r, kf, cf, ρf, μf, ϵ=0.0)
...
function resistance_fluid(V̇::Real, r::Real, kf::Real, cf::Real, ρf::Real, μf::Real, ϵ::Real=0.0)
