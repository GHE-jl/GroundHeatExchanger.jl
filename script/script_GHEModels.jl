"""
Script showing the analytical models from GHEModels.jl.
"""
...
@time g_mfls = mfls(t, ks, Cs, rb, H, D, vD)
@time g_scwm = scwm(t, ks, Cs, rb, H, V, 0.01)

col = palette(:tab10)
plot(t, g_ils, lw=3, lc=col[1], label="IlS")
plot!(t, g_fls, lw=3, lc=col[2], label="FlS")
plot!(t, g_mfls, lw=3, lc=col[3], label="MFlS")
plot!(t, g_scwm, lw=3, lc=col[4], label="SCWM")
plot!(xaxis="Time (s)",
    yaxis="g (-)",
    xscale=:log10,
    framestyle=:box,
